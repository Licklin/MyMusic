...
public class SongLrcViewModel extends MusicViewModel {
    ...
    public void setPlaybackModeRes(int modeNow) {
        if (modeNow < 1 || modeNow > 3) {
            Log.e(TAG, "播放模式错误代码 " + modeNow);
            return;
        }
    ...
    public void SyncMusicInformation() {
        ...
        //音量
        int volume = (int) lastMetadata.getLong(MyAudioManager.MyMusic_CUSTOM_ACTION_CURRENT_VOLUME);
        int volumeMax = (int) lastMetadata.getLong(MyAudioManager.MyMusic_CUSTOM_ACTION_MAX_VOLUME);
        ...
        setPlaybackModeRes((int) lastMetadata.getLong(
                MediaPlayerManager.MyMusic_CUSTOM_ACTION_PLAYBACK_MODE_CHANGE));
        //显示处理成唱片样式的歌曲封面图片
    ...
    public String getMusicName() {
        return this.musicTitle + " - " + this.musicArtist;
    }
    ...
    private class volumeSeekBar implements SeekBar.OnSeekBarChangeListener {
        ...
        public void onStopTrackingTouch(SeekBar seekBar) {
            ...
            bundle.putInt(MyAudioManager.MyMusic_CUSTOM_ACTION_CURRENT_VOLUME, seekBar.getProgress());
            mMediaControllerCompat.getTransportControls().sendCustomAction(
                    MyAudioManager.MyMusic_CUSTOM_ACTION_CURRENT_VOLUME, bundle);
        }
